      ...
      <div className="mx-auto grid w-full max-w-[1380px] gap-10 px-4 pb-14 pt-6 sm:px-6 lg:min-h-screen lg:grid-cols-[minmax(0,0.7fr)_minmax(0,1.3fr)] lg:items-center lg:px-8 lg:py-10">
        <section className="landing-reveal flex max-w-[28rem] flex-col gap-7">
          ...
          <div className="space-y-4">
            ...
            <h1 className="font-display max-w-[10ch] text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">
              Continue with email.
            </h1>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
            {destinations.map((destination) => (
              <div
                className="rounded-[var(--radius-md)] border border-[var(--stroke-soft)] px-4 py-3 shadow-[var(--shadow-soft)]"
                key={destination.label}
                style={{ backgroundColor: destination.tone }}
              >
                <div className="text-[0.6rem] font-semibold uppercase tracking-[0.1em] text-[var(--muted)]">
                  {destination.label}
                </div>
                <div className="font-display mt-2 text-lg font-semibold tracking-[-0.03em] text-[var(--ink)]">
                  {destination.value}
                </div>
              </div>
            ))}
        ...
        <section className="landing-reveal landing-reveal-delay-1 lg:justify-self-end">
          <div className="paper-grain relative overflow-hidden rounded-[var(--radius-lg)] border border-[var(--stroke)] bg-[color-mix(in_srgb,var(--surface)_84%,white_16%)] p-3 shadow-[var(--shadow-lifted)] sm:p-4">
            <div className="absolute inset-0 bg-[linear-gradient(150deg,transparent,rgba(255,255,255,0.2)_45%,transparent_64%)] opacity-70" />
            <div className="relative max-w-[42rem] rounded-[var(--radius-md)] border border-[var(--stroke-soft)] bg-[var(--surface-weak)] p-6 sm:p-8">
