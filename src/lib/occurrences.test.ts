...
import {
  ...
  generateOccurrenceDayKeys,
  type RepeatRule,
} from "@/lib/occurrences";

const timeZone = "Europe/Warsaw";

describe("generateOccurrenceDayKeys", () => {
  it("returns weekly single-day occurrences", () => {
    const dayKeys = generateOccurrenceDayKeys({
    ...
    expect(dayKeys).toEqual(["2026-03-02"]);
  });
  ...
  it("returns two-day weekly occurrences", () => {
    ...
    expect(dayKeys).toEqual(["2026-03-06", "2026-03-07"]);
  });
  ...
  it("includes span days that start before the range", () => {
    ...
    expect(dayKeys).toEqual(["2026-03-02", "2026-03-08"]);
  });
  ...
  it("keeps overlaps when final start is before range start", () => {
    ...
    expect(dayKeys).toEqual(["2026-01-05", "2026-01-06", "2026-01-07"]);
  });
...
describe("generateOccurrenceDayEntries", () => {
  it("returns day entries with shared occurrence start identity for overlapping daily spans", () => {
