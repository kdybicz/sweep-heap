...
export const ensureChoresTable = async () => {
  ...
  await pool.query(
    "create table chore_occurrence_overrides (id serial primary key, chore_id integer not null references chores(id) on delete cascade, occurrence_date date not null, status text not null, closed_reason text, undo_until timestamptz, updated_at timestamptz not null default now(), unique(chore_id, occurrence_date))",
  );
};
