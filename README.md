...
- `make install` uses Yarn's `--immutable` mode, and GitHub Actions runs separate `make lint`, `make test`, and `make typecheck` jobs with a shared setup action and Yarn cache.
