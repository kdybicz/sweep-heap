...
- Surprise to watch for: Better Auth session schema stores `active_household_id`, but current household lookups in `src/lib/repositories/household-repository.ts` ignore it and derive the active household from the latest active membership by `joined_at`. Do not assume session-selected household state is honored yet.
