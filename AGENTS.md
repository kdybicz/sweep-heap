...
- Surprise to watch for: `scripts/with-agent-env.mjs` is a lightweight parser, not full dotenv semantics (limited quoting/spacing/comment support), and it applies `.env.agent.local` after `AGENT_ENV_FILE`, so local overrides can unexpectedly shadow external overrides.
