...
- Surprise to watch for: the undo window duration is hardcoded in multiple places (`src/lib/services/chore-service.ts` and `src/app/household/board/useHouseholdChoreActions.ts`), so changing it in one place can silently desync API and UI behavior.
- Surprise to watch for: Skip/Snooze buttons in `src/app/household/board/components/ChoreDetailsModal.tsx` are UI placeholders with no handlers; they render as interactive controls but currently perform no action.
